...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // monthly total escrow report..

        $escrowChart = Escrow::selectRaw("COUNT(*) as total_escrow , DATE(created_at) as date_name")->whereMonth('created_at',date('m'))->orderBy('created_at','desc')->groupByRaw('DATE(created_at)')->pluck('total_escrow','date_name');
        $escrow['labels'] = $escrowChart->keys();
        $escrow['values'] = $escrowChart->values();


        // UserLogin Report Graph
        $newTickets = SupportTicket::with('user')->orderBy('created_at', 'desc')->whereStatus(0)->limit(5)->get();
        return view('admin.dashboard', compact('pageTitle', 'widget', 'deposit', 'withdrawals', 'userLogins', 'escrow', 'newTickets'));
    }
